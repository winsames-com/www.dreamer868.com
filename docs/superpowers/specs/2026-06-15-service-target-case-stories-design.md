...
- `claude` 訂閱帳戶（改編引擎；無需 Anthropic API key）：本機互動登入，或伺服器用 `claude setup-token` → `CLAUDE_CODE_OAUTH_TOKEN`
- 「司法院資料開放平臺」(https://opendata.judicial.gov.tw/) 註冊帳號 → 填入 `pipeline/.env`（`JUD_USER` / `JUD_PASS`）
- 安裝 crontab 指向 `pipeline/cron.sh`（對齊台灣 0–6 點；UTC 機器用 17:00 或 `TZ=Asia/Taipei`）
- 目標部署：伺服器（與機器無關，詳見 pipeline/README「在伺服器部署」）
